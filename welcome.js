...
module.exports = function (client) {
    ...
    client.on("guildMemberAdd", async member => {
      ...
      const welcomeembed = new Discord.MessageEmbed()
        ...
        .setDescription(`
      :small_blue_diamond: **Welcome** : <@${member.id}>   :small_blue_diamond:\n
      :small_blue_diamond: **Welcome to** \`${member.guild.name}\` :small_blue_diamond:\n
      :small_blue_diamond: **You Member** : \`${member.guild.memberCount}\`:small_blue_diamond:`)
        .setImage("attachment://welcome-image.png")
        .attachFiles(attachment);
      //define the welcome channel
      const channel = member.guild.channels.cache.find(ch => ch.id === bot.CHANNEL_WELCOME);
      //send the welcome embed to there
      channel.send(welcomeembed);
      //member roles add on welcome every single role
      let roles = bot.ROLES_WELCOME;
